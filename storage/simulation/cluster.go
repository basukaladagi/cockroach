...
func (c *Cluster) performActions() {
	...
	for _, rangeID := range c.rangeIDs {
		...
		case storage.AllocatorAdd:
			...
			fmt.Fprintf(c.actionWriter, "Range %d - Add %d\n", rangeID, newStoreID)
			r.addReplica(c.stores[newStoreID])
		case storage.AllocatorRemoveDead:
			// TODO(bram): implement this.
			fmt.Fprintf(c.actionWriter, "Range %d - Repair\n", rangeID)
		case storage.AllocatorRemove:
			// TODO(bram): implement this.
			fmt.Fprintf(c.actionWriter, "Range %d - Remove\n", rangeID)
		case storage.AllocatorNoop:
			if rebalance {
				// TODO(bram): implement this.
				fmt.Fprintf(c.actionWriter, "Range %d - Rebalance\n", rangeID)
			}
		}
	}
}
...
func (c *Cluster) OutputEpochHeader() {
	...
	w.Init(c.epochWriter, 0, 8, 0, '\t', 0)
	fmt.Fprintf(w, "Store:\t")
	for _, storeID := range c.storeIDs {
		fmt.Fprintf(w, "%d\t", storeID)
	}
	w.Flush()
}
...
func (c *Cluster) OutputEpoch() {
	...
	for _, storeID := range c.storeIDs {
		...
		fmt.Fprintf(w, "%.0f%%\t", float64(capacity.Available)/float64(capacity.Capacity)*100)
	}
	fmt.Fprintf(w, "\n")
	w.Flush()
}
